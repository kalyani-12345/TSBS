document.addEventListener("DOMContentLoaded", function () {
  ...
  const totalWithTaxes = (subtotal + parseFloat(sgst) + parseFloat(cgst)).toFixed(2);

  // Update tax values and total price
  document.getElementById("sgst").innerText = `₹${sgst}`;
  document.getElementById("cgst").innerText = `₹${cgst}`;
  document.getElementById("total-price").innerText = `₹${totalWithTaxes}`;
  ...
  document.getElementById("download-pdf").addEventListener("click", function () {
    ...
    doc.text("SGST (9%): ₹" + document.getElementById("sgst").innerText.split("₹")[1], 10, yPos);
    doc.text("CGST (9%): ₹" + document.getElementById("cgst").innerText.split("₹")[1], 10, yPos + 10);
    doc.text("Total Price: ₹" + document.getElementById("total-price").innerText.split("₹")[1], 10, yPos + 20);
  ...
  document.getElementById("send-sms").addEventListener("click", function () {
    ...
    const totalAmount = document.getElementById("total-price").innerText.split("₹")[1];
